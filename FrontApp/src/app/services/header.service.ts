...
export class HeaderService {
  headerImages: string[] = [
    ...
    'https://images.unsplash.com/photo-1600091106710-7be582d40984?q=80&w=2072&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  ...
  getHeaderImageUrlByIndex(index: number): string {
    if (index < this.headerImages.length) {
      return this.headerImages[index];
    } else {
      return this.headerImages[0];
    }
  }

  getHeaderImageUrl(): string {
    const ind = Math.floor(Math.random() * this.headerImages.length);
    const header: string = this.headerImages[ind];
    return header;
  }
}
